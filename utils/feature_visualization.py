...
def visualize_features(parameters_file, dataset, classe, print_title=True):
	...
	plt.savefig(f'../figures/mean_features_{classe}.png')
	plt.show()
...
def get_correlation_features(model, dataset):
	...
	for (c1, c2) in zip(dataset.label_list, dataset.label_list): 
		correlations[f'{c1}_{c2}'] = [[] for i in range(counter+1)]
		for c in range(counter+1):
			for img1, img2 in zip(features[c1][c], features[c2][c]):
				if c1 != c2:
					correlations[f'{c1}_{c2}'][c].append(get_correlation(img1, img2))
