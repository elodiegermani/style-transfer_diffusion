...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6):
        ...
        layers = []
        layers.append(nn.Conv3d(c_dim+1, conv_dim, kernel_size=7, stride=1, padding=3, bias=False))
        ...
        k_sizes_d = [4, 4]
        for i in range(2):
            layers.append(nn.Conv3d(curr_dim, curr_dim*2, kernel_size=k_sizes_d[i], stride=2, padding=1, bias=False))
        ...
        k_sizes_u = [4,4]
        for i in range(2):
            layers.append(nn.ConvTranspose3d(curr_dim, curr_dim//2, kernel_size=k_sizes_u[i], stride=2, padding=1, bias=False))
            layers.append(nn.InstanceNorm3d(curr_dim//2, affine=True, track_running_stats=True))
            layers.append(nn.ReLU(inplace=True))
            curr_dim = curr_dim // 2
...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_size=56, conv_dim=64, c_dim=5, repeat_num=4):
        ...
        self.main = nn.Sequential(*layers)
        self.conv1 = nn.Conv3d(curr_dim, 1, kernel_size=4, stride=1, padding=1, bias=False)
